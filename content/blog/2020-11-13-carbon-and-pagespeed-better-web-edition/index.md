...
The [original post](/blog/carbon-and-pagespeed/) can be found on this websites for comparison.

## Methodology
The same selection of websites as the original study were chosen, and once again tested using a series of automated tools. This time, the metrics tested were expanded to cover all of those measured by the Better Web Alliance. In a further refinement of the original analysis, the Homepage and a single article were also tested for each site, compared to just the homepage in the original study.
...
- [WAVE](https://wave.webaim.org/) - checks for compliance with Web Content Accessibility Guidelines (WCAG). Reports number of Errors, Alerts (potential problems) and Contrast Errors (colour contrast between text and background). Lower is better.
...
The results, particularly the shift since the last measurement are very interesting. Since the first analysis, Pagespeed Insights have updated their measurement metrics, and almost all of the sites have regressed on those scores; though some very much more so than the others, with the Daily Mirror losing a whole 45 points on desktop, compared to the 1 for this site.

Total page weight and carbon (where data is available) seem to be a pretty even split between improvement and regression. The 2 notable outliers here being The Times, which has lost a whole 2.5mb of weight from the it's homepage, and The Sun which has gained an enormous 10.1mb on page weight.
...
Privacy scores accross the Newspaper websites also produce cause for alarm. Sharing customer data with third parties is universally practised amongst the sites, with all but a handful allowing google analytics to track visitors across the web. The Sun and The Times both load tracking scripts designed to try and bypass tracking cookie blockers and The Independent, Daily Star, and The Times are likely all tracking their visitors' mouse clicks and keyboard input.
